taskset -c 12-31 accelerate launch --config_file ckpts/multi_gpu.yaml src/f5_tts/train/train.py \
    ...
    datasets.batch_size_per_gpu=55000 \
    datasets.batch_size_type=frame \
    datasets.max_samples=200 \
    ...
    ckpts.save_dir=ckpts/f5tts_vi_v1_base_ft_ali__adapt_alpha \
    ...
    \
    model.mel_attn_alpha_scheduler.enabled=True \
    model.mel_attn_alpha_scheduler.num_steps=50000
